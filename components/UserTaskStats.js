...
const UserTaskStats = ({ tasks }) => {
  const totalRemainingTime = tasks.reduce((acc, task) => acc + (task.remainingTime ?? 0), 0);
  const tasksWithoutNull = tasks.filter((task) => task.remainingTime != null);
  const averageRemainingTime = tasksWithoutNull.length > 0 ? totalRemainingTime / tasksWithoutNull.length : 0;
  ...
  return (
    <View style={styles.statsContainer}>
      ...
      <Text style={styles.statsText}>Durée moyenne: {averageRemainingTime.toFixed(2)}</Text>
      <View style={styles.labelsContainer}>
        <View style={styles.labelsContainer}>
        {Object.entries(importanceLabels).map(([code, { label, count }]) => (
          <TaskLabel key={code} label={`${label}: ${count}`} color={Colors.flags[code]} />
        ))}
        </View>
        <View style={styles.labelsContainer}>
        {Object.entries(statusLabels).map(([code, { label, count }]) => (
          <TaskLabel key={code} label={`${label}: ${count}`} color={Colors.status[code]} />
        ))}
        </View>
      </View>
    </View>
